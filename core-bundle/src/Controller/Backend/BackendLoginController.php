...
class BackendLoginController extends AbstractController
{
    public function __invoke(Request $request): Response
    {
        ...
        $template->REQUEST_TOKEN = REQUEST_TOKEN;

        $backendThemes = $this->get(BackendThemes::class);

        if ('flexible' !== $themeName && null === $backendThemes->getTheme($themeName)) {
        ...
        return $template->getResponse()->setContent(
            $this->get('twig')->render(
                sprintf('@ContaoCore/Backend/Layout/%s.html.twig', ltrim($template->getName(), 'be_')),
                $template->getData()
            )
        );
    }
